...
function Remove-SitecoreLocal {
    ...
    Param (
		...
		# Name of Sitecore Local instance [default prefix from ConfigurationFile]
	...
	process {
		try {
			if($PSCmdlet.ShouldProcess($ConfigurationFile)) {
				...
				Remove-SitecoreLocalServices $name # -ConfigurationFile $ConfigurationFile
				...
				$configRoot = $assets.configurationRoot

				#Uninstall-SitecoreConfiguration @configurationParams

				Set-Location $configRoot

				#. .\uninstall-xp0 -Force -ErrorAction SilentlyContinue
				
				# todo: stop windows services:
				#Stop-SitecoreLocal 

				Remove-SitecoreLocalSite $ConfigurationFile
