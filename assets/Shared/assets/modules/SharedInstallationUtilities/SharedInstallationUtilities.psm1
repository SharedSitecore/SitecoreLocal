...
Function Add-DatabaseUser {
	...
	Write-Host "#####################################################`n$($sqlVariables | Format-Table -AutoSize -Wrap| Out-String)#####################################################`n" -ForegroundColor White
	Write-Verbose "#####################################################`n$($sqlVariables | Format-Table -AutoSize -Wrap| Out-String)#####################################################`n"
...
Function Kill-DatabaseConnections {
	...
	Write-Verbose "Sql File: $sqlVariables"
	Write-Host "Sql File: $sqlVariables"

	#Write-Error "ERROR invalid Sql File:$sqlVariables"
	#Exit 1
	
	$sqlFile = Join-Path (Resolve-Path "..\..\..") "\database\killdatabaseconnections.sql"

	Write-Verbose "Sql File: $sqlFile"
	Write-Host "Sql File: $sqlFile"
	#if (!(Test-Path $sqlFile)) { Write-Error "ERROR invalid Sql File:$sqlFile" }
	#Write-Error "ERROR invalid Sql File:$sqlFile"
	#Exit 1
	Invoke-Sqlcmd -Variable $sqlVariables -UserName $SqlAdminUser -Password $SqlAdminPassword -ServerInstance $SqlServer -InputFile $sqlFile
}
...
Function Get-ObjectMembers {
	...
	$obj | Get-Member -MemberType NoteProperty | ForEach-Object {
		Write-Verbose "Get-ObjectMembers:$_.Name"
		$key = $_.Name
